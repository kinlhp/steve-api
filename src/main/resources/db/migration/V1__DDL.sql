...
USE `steve`$$
DROP TRIGGER IF EXISTS `steve`.`pessoa_tipo_BEFORE_INSERT` $$
USE `steve`$$
CREATE DEFINER = CURRENT_USER TRIGGER `steve`.`pessoa_tipo_BEFORE_INSERT` BEFORE INSERT ON `pessoa` FOR EACH ROW BEGIN
	DECLARE tipo_sistema_unique CONDITION FOR SQLSTATE '23000';
...
USE `steve`$$
DROP TRIGGER IF EXISTS `steve`.`pessoa_ie_rg_BEFORE_UPDATE` $$
USE `steve`$$
CREATE DEFINER = CURRENT_USER TRIGGER `steve`.`pessoa_ie_rg_BEFORE_UPDATE` BEFORE UPDATE ON `pessoa` FOR EACH ROW BEGIN
	DECLARE ie_rg_non_isento_unique CONDITION FOR SQLSTATE '23000';
...
USE `steve`$$
DROP TRIGGER IF EXISTS `steve`.`pessoa_tipo_BEFORE_UPDATE` $$
USE `steve`$$
CREATE DEFINER = CURRENT_USER TRIGGER `steve`.`pessoa_tipo_BEFORE_UPDATE` BEFORE UPDATE ON `pessoa` FOR EACH ROW BEGIN
	DECLARE tipo_sistema_unique CONDITION FOR SQLSTATE '23000';
    IF(EXISTS(SELECT 1 FROM `pessoa` WHERE NEW.`tipo` = 'SISTEMA' AND `tipo` = NEW.`tipo`)) THEN
--      MESSAGE_TEXT = VARCHAR(128)
--      SET @current_schema_name_text = (SELECT SCHEMA());
--      SET @unmodifiable_message_text = CONCAT('Cannot delete or update a parent row: a foreign key constraint fails (`', @current_schema_name_text, '`.`credencial`, CONSTRAINT `FK_credencial_funcionario` FOREIGN KEY (`funcionario`) REFERENCES `pessoa` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION)');
        SET @unmodifiable_message_text = CONCAT('Cannot delete or update a parent row having `tipo` = ''SISTEMA''');
        SIGNAL tipo_sistema_unique SET MESSAGE_TEXT = @unmodifiable_message_text, MYSQL_ERRNO = 1451;
	END IF;
END;$$
